...
addToRunTimeSelectionTable(option, actuatorLineSource, dictionary);
} // namespace fv
} // namespace Foam
...
bool Foam::fv::actuatorLineSource::read(const dictionary &dict) {
    if (cellSetOption::read(dict)) {
        ...
        bool writeForceField = coeffs_.lookupOrDefault("writeForceField", true);
...
void Foam::fv::actuatorLineSource::createElements() {
    ...
    if (nElements_ % nGeometrySegments) {
        ...
        FatalErrorIn("void actuatorLineSource::createElements()")
            ...
            << "number of actuator line geometry segments" << abort(FatalError);
    }
    ...
    forAll(elements_, i) {
        ...
        position = point1 + segment / nElementsPerSegment * pointIndex +
        ...
        pitch = pitch1 + deltaPitchTotal / nElementsPerSegment * pointIndex +
        ...
        actuatorLineElement *element =
            new actuatorLineElement(name, dict, mesh_);
...
void Foam::fv::actuatorLineSource::calcEndEffects() {
    ...
    forAll(elements_, i) { elements_[i].setEndEffectFactor(factors[i]); }
...
void Foam::fv::actuatorLineSource::harmonicPitching() {
    ...
    if (t != lastMotionTime_) {
        scalar omega =
            reducedFreq_ * 2 * mag(freeStreamVelocity_) / chordLength_;
        scalar dt = mesh_.time().deltaT().value();
        scalar deltaPitch = degToRad(pitchAmplitude_) *
                            (sin(omega * t) - sin(omega * (t - dt)));
        pitch(deltaPitch);
        lastMotionTime_ = t;
    }
}
...
Foam::fv::actuatorLineSource::actuatorLineSource(const word &name,
                                                 const word &modelType,
                                                 const dictionary &dict,
                                                 const fvMesh &mesh)
    : cellSetOption(name, modelType, dict, mesh), force_(vector::zero),
      forceField_(
          IOobject("force." + name_, mesh_.time().timeName(), mesh_,
                   IOobject::NO_READ, IOobject::AUTO_WRITE),
          mesh_,
          dimensionedVector("force", dimForce / dimVolume, vector::zero)),
      writePerf_(coeffs_.lookupOrDefault("writePerf", false)),
      lastMotionTime_(mesh.time().value()), endEffectsActive_(false) {
    read(dict_);
    createElements();
    if (writePerf_) {
        createOutputFile();
...
void Foam::fv::actuatorLineSource::pitch(scalar radians) {
    forAll(elements_, i) { elements_[i].pitch(radians); }
}

void Foam::fv::actuatorLineSource::pitch(scalar radians, scalar chordFraction) {
    forAll(elements_, i) { elements_[i].pitch(radians, chordFraction); }
}

void Foam::fv::actuatorLineSource::translate(vector translationVector) {
    forAll(elements_, i) { elements_[i].translate(translationVector); }
}

void Foam::fv::actuatorLineSource::setSpeed(vector point, vector axis,
                                            scalar omega) {
    forAll(elements_, i) { elements_[i].setSpeed(point, axis, omega); }
}

void Foam::fv::actuatorLineSource::scaleVelocity(scalar scale) {
    forAll(elements_, i) { elements_[i].scaleVelocity(scale); }
}

void Foam::fv::actuatorLineSource::setOmega(scalar omega) {
    forAll(elements_, i) { elements_[i].setOmega(omega); }
}

const Foam::vector &Foam::fv::actuatorLineSource::force() { return force_; }

const Foam::volVectorField &Foam::fv::actuatorLineSource::forceField() {
    return forceField_;
}

PtrList<Foam::fv::actuatorLineElement> &
Foam::fv::actuatorLineSource::elements() {
    return elements_;
}

Foam::vector Foam::fv::actuatorLineSource::moment(vector point) {
    vector moment(vector::zero);
    forAll(elements_, i) { moment += elements_[i].moment(point); }
...
void Foam::fv::actuatorLineSource::addSup(fvMatrix<vector> &eqn,
                                          const label fieldI) {
    ...
    Info << "Force (per unit density) on " << name_ << ": " << endl
         << force_ << endl
         << endl;
...
void Foam::fv::actuatorLineSource::addSup(fvMatrix<scalar> &eqn,
                                          const label fieldI) {
    // If harmonic pitching is active, do harmonic pitching
    if (harmonicPitchingActive_) {
        harmonicPitching();
...
void Foam::fv::actuatorLineSource::addSup(const volScalarField &rho,
                                          fvMatrix<vector> &eqn,
                                          const label fieldI) {
    // If harmonic pitching is active, do harmonic pitching
    if (harmonicPitchingActive_) {
